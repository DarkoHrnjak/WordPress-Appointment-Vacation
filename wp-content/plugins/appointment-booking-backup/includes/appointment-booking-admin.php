...
function appointment_booking_admin_page() {
	?>
    <div class="wrap">
        <h1><?php esc_html_e( 'Appointment Booking', 'appointment-booking' ); ?></h1>
        <p><?php esc_html_e( 'Manage your appointment settings here.', 'appointment-booking' ); ?></p>
...
function appointment_booking_appointments_page_content() {
	...
	?>
    <div class="wrap">
        <h1><?php esc_html_e( 'Appointments', 'appointment-booking' ); ?></h1>
        <p><?php esc_html_e( 'View appointments here.', 'appointment-booking' ); ?></p>
        ...
        <table class="appointments-table">
			...
			<?php if ( ! empty( $appointments ) ) : ?>
				<?php foreach ( $appointments as $appointment ) : ?>
                    <tr>
                        ...
                        <td>
                            <a href="<?php echo admin_url('admin.php?page=appointment-booking-edit-appointment&id=' . $appointment['id']); ?>" class="button button-secondary">Edit</a>
                            <form method="post" action="" style="display:inline; margin-left:5px;">
                                ...
                                <button type="submit" class="button button-secondary" onclick="return confirm('Are you sure you want to delete this appointment?');">Delete</button>
                            </form>
                        </td>
                    </tr>
				<?php endforeach; ?>
			<?php else : ?>
                <tr>
                    <td colspan="6"><?php esc_html_e( 'No appointments found.', 'appointment-booking' ); ?></td>
                </tr>
			<?php endif; ?>
            </tbody>
        </table>
        ...
        <form method="post" action="">
            <table class="form-table">
                <tr>
                    ...
                    <td><input type="date" name="appointment_date" id="appointment_date" value="<?php echo esc_attr( $appointment['appointment_date'] ); ?>" required></td>
                </tr>
                <tr>
                    <th><label for="appointment_time">Time</label></th>
                    <td><input type="time" name="appointment_time" id="appointment_time" value="<?php echo esc_attr( $appointment['appointment_time'] ); ?>" required></td>
                </tr>
                <tr>
                    <th><label for="appointment_name">Name</label></th>
                    <td><input type="text" name="appointment_name" id="appointment_name" value="<?php echo esc_attr( $appointment['appointment_name'] ); ?>" required></td>
                </tr>
                <tr>
                    <th><label for="appointment_phone">Phone</label></th>
                    <td><input type="text" name="appointment_phone" id="appointment_phone" value="<?php echo esc_attr( $appointment['appointment_phone'] ); ?>" required></td>
                </tr>
                <tr>
                    <th><label for="appointment_comments">Comments</label></th>
                    <td><textarea name="appointment_comments" id="appointment_comments" rows="4"><?php echo esc_textarea( $appointment['appointment_comments'] ); ?></textarea></td>
                </tr>
            </table>
            <p class="submit">
...
function appointment_booking_vacation_page_content() {
	...
	?>
    <div class="wrap">
        <h1><?php esc_html_e( 'Set Vacation Dates', 'appointment-booking' ); ?></h1>
        <form method="post" action="">
        ...
        <h2><?php esc_html_e( 'Existing Vacations', 'appointment-booking' ); ?></h2>
        <table class="vacations-table wp-list-table widefat fixed striped">
            <thead>
            <tr>
			...
			<?php if ( ! empty( $vacation_dates ) ) : ?>
				<?php foreach ( $vacation_dates as $vacation ) : ?>
                    <tr>
                        ...
                        <td>
                            <!-- Edit Button -->
                            <a href="<?php echo admin_url('admin.php?page=appointment-booking-edit-vacation&id=' . $vacation->id); ?>" class="button button-secondary">Edit</a>
                            <!-- Delete Button -->
                            <form method="post" action="" style="display:inline; margin-left:5px;">
                                <input type="hidden" name="delete_vacation" value="<?php echo esc_attr($vacation->id); ?>">
                                <button type="submit" class="button button-secondary" onclick="return confirm('Are you sure you want to delete this vacation?');">Delete</button>
                            </form>
                        </td>
                    </tr>
				<?php endforeach; ?>
			<?php else : ?>
                <tr>
                    <td colspan="3"><?php esc_html_e( 'No vacation dates set.', 'appointment-booking' ); ?></td>
                </tr>
			<?php endif; ?>
            </tbody>
        </table>
...
function appointment_booking_edit_vacation_page() {
    ...
    <div class="wrap">
        ...
        <form method="post" action="">
            <table class="form-table">
                <tr>
                    <th><label for="vacation_start_date">Start Date</label></th>
                    <td><input type="date" name="vacation_start_date" id="vacation_start_date" value="<?php echo esc_attr( $vacation['vacation_start_date'] ); ?>" required></td>
                </tr>
                <tr>
                    <th><label for="vacation_end_date">End Date</label></th>
                    <td><input type="date" name="vacation_end_date" id="vacation_end_date" value="<?php echo esc_attr( $vacation['vacation_end_date'] ); ?>" required></td>
                </tr>
            </table>
            <p class="submit">
