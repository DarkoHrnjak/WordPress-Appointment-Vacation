...
function appointment_booking_form_shortcode() {
	...
	<div class="appointment-booking-form">
		...
		<p class="subtitle"><?php esc_html_e( 'Pre-book your slot. Available times are between 9am and 4pm.', 'appointment-booking' ); ?></p>
		<form method="post" action="">
			...
			<div class="form-group">
				...
				<input type="tel" id="appointment_phone" name="appointment_phone" class="appointment-phone-number" inputmode="tel" autocomplete="tel" pattern="[0-9+\-\s()]+" title="Please enter a valid phone number" required>
			</div>
			...
			<div class="form-group">
				...
				<select id="appointment_time" name="appointment_time" required disabled>
					<option value=""><?php esc_html_e( 'Please select a Date first', 'appointment-booking' ); ?></option>
...
function handle_appointment_form_submission() {
	if ( isset( $_POST['submit_appointment'] ) ) {
		...
		if ( strtotime( $appointment_date ) < strtotime( date( 'Y-m-d' ) ) ) {
			echo '<div class="error">' . esc_html__( 'Sorry, you cannot book an appointment for a past date.', 'appointment-booking' ) . '</div>';
			return;
		}
